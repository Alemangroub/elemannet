...
// PUT: /api/projects/{projectId}/costs/contractor-wages/{tableId}
export const PUT = async ({ params, request }) => {
    const { tableId } = params;
    try {
        const updates = await request.json();
        await db.collection('contractor_wages_tables').doc(tableId).update({
            ...updates,
            updatedAt: new Date()
        });
...
// DELETE: /api/projects/{projectId}/costs/contractor-wages/{tableId}
export const DELETE = async ({ params }) => {
    const { tableId } = params;
    try {
        await db.collection('contractor_wages_tables').doc(tableId).delete();
